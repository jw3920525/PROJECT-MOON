...
class PagerApp {
  constructor() {
    ...
    // Gemini AI 지령 하루 1회 자동 생성
    if (this.config.gemini_auto && this.config.gemini_api_key) {
      setTimeout(() => this.autoGenerateGeminiOnStartup(), 4000);
    }
  }
  ...
  initDOM() {
    this.dom = {
      ...
      btnGeminiGenerate: document.getElementById('btn-gemini-generate'),
      toggleGeminiAuto: document.getElementById('toggle-gemini-auto'),
  ...
  bindEvents() {
    ...
    // 12-1. Gemini AI 지령 생성 버튼 & 자동 생성 토글
    if (this.dom.btnGeminiGenerate) {
      this.dom.btnGeminiGenerate.addEventListener('click', () => this.handleGeminiGenerateClick());
    }
    if (this.dom.toggleGeminiAuto) {
      this.dom.toggleGeminiAuto.addEventListener('change', (e) => {
        this.saveConfig({ gemini_auto: e.target.checked });
        this.showToast(e.target.checked ? "매일 자동 생성이 활성화되었습니다." : "자동 생성이 비활성화되었습니다.");
      });
    }
  ...
  openModal() {
    ...
    if (this.dom.toggleGeminiAuto) {
      this.dom.toggleGeminiAuto.checked = !!this.config.gemini_auto;
    }
    this.applyCustomColors(this.config.font_color, this.config.bg_color);
  ...
  saveSettingsFromModal() {
    ...
    const newConfig = {
      ...
      gemini_auto: this.dom.toggleGeminiAuto ? this.dom.toggleGeminiAuto.checked : !!this.config.gemini_auto,
    };
  ...
  // ── Gemini AI 지령 자동 생성 ──
  // 관리부 지령 톤(프로젝트 문 세계관 분위기 차용)의 창작 지령을
  // messages.json과 동일한 {stage, messages:[{text, time_info}]} 구조로 생성한다.
  buildGeminiRequestBody(hint) {
    const systemPrompt = [
      "너는 '프로젝트 문(Project Moon)' 세계관(림버스 컴퍼니, 루시드 드림 등)에서",
      "'관리부'가 소속 임원에게 페이저(삐삐) 단말기로 내려보내는 '지령'을 작성하는 역할이다.",
      "",
      "규칙:",
      "- 사무적이고 간결한 관리부식 명령체를 쓴다. (예: '~할 것.', '~바람.', '~요망.')",
      "- 각 지령 문구(text)는 28자 이내로 짧게 작성한다.",
      "- time_info 필드에는 지령의 긴급도/분류를 2~6자 태그로 넣는다. (예: '긴급', '경고', '일반', '보고', '대기')",
      "- 실존 캐릭터 이름이나 대사를 그대로 재현하지 말고, 분위기만 차용한 창작 지령을 만든다.",
      "- 3개 단계(stage 1~3)를 만들고, 각 단계마다 서로 다른 소재의 지령을 2개씩 작성한다.",
      "- 반드시 한국어로만 작성한다."
    ].join("\n");

    let userPrompt = "위 규칙에 맞는 지령 세트를 생성해라.";
    if (hint && hint.trim()) {
      userPrompt += ` 이번 지령의 전반적인 상황: ${hint.trim()}`;
    }
  ...
  // Gemini API를 직접 호출해서 stage 배열을 반환한다. 실패 시 예외를 던진다.
  async generateGeminiStages(apiKey, hint) {
    if (!apiKey || apiKey.trim().length < 10) {
      throw new Error("Gemini API 키를 입력해주세요.");
    }

    const model = "gemini-2.5-flash"; // 필요시 최신 모델명으로 교체 가능
    const url = `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${encodeURIComponent(apiKey.trim())}`;
    const body = this.buildGeminiRequestBody(hint);
    ...
    let stages;
    try {
      stages = JSON.parse(rawText);
    } catch (e) {
      throw new Error("Gemini 응답을 JSON으로 해석하지 못했습니다.");
    }

    if (!Array.isArray(stages) || stages.length === 0) {
      throw new Error("Gemini가 유효한 지령 세트를 생성하지 못했습니다.");
    }

    // 형식 정리(30자 초과 방지 등 기존 규칙과 동일하게 맞춤)
    return stages.map((s, idx) => ({
      stage: idx + 1,
      messages: (s.messages || []).map(m => ({
        text: this.truncateText(String(m.text || "").trim(), 30),
        time_info: String(m.time_info || "").trim()
      })).filter(m => m.text)
    })).filter(s => s.messages.length > 0);
  }

  // 설정 모달의 [AI로 지령 생성] 버튼 클릭 시: 결과를 에디터에 채워 넣고 검토를 유도한다.
  async handleGeminiGenerateClick() {
    const apiKey = this.dom.inputGeminiKey ? this.dom.inputGeminiKey.value.trim() : "";
    const hint = this.dom.inputGeminiHint ? this.dom.inputGeminiHint.value.trim() : "";
    ...
    if (this.dom.btnGeminiGenerate) {
      this.dom.btnGeminiGenerate.textContent = "생성 중...";
      this.dom.btnGeminiGenerate.disabled = true;
    }

    try {
      const stages = await this.generateGeminiStages(apiKey, hint);
      this.customStages = stages;
      this.renderCustomStageCards();
      this.saveConfig({ gemini_api_key: apiKey, gemini_hint: hint });
      this.showToast("AI 지령 생성 완료. 검토 후 [적용하기]를 눌러주세요.");
    } catch (e) {
      console.error("Gemini 지령 생성 실패:", e);
      alert(`AI 지령 생성 실패:\n${e.message}`);
    } finally {
      if (this.dom.btnGeminiGenerate) {
        this.dom.btnGeminiGenerate.textContent = "AI로 지령 생성";
        this.dom.btnGeminiGenerate.disabled = false;
      }
    }
  }

  // 앱 실행 시 자동 생성(하루 1회). 사용자 개입 없이 바로 적용한다.
  async autoGenerateGeminiOnStartup() {
    if (!this.config.gemini_auto || !this.config.gemini_api_key) return;

    const lastGenKey = localStorage.getItem('limbus_gemini_last_gen_day');
    if (lastGenKey === this.todayKey) return; // 오늘 이미 생성함

    try {
      const stages = await this.generateGeminiStages(this.config.gemini_api_key, this.config.gemini_hint);
      this.saveStoredMessages(stages);
      localStorage.setItem('limbus_gemini_last_gen_day', this.todayKey);
      this.showToast("오늘의 AI 지령이 자동 생성되었습니다.");
    } catch (e) {
      console.warn("Gemini 자동 생성 실패:", e);
    }
  }
