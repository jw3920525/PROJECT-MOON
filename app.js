...
class PagerApp {
  ...
  buildGeminiSingleMessageRequestBody(hint) {
    const systemPrompt = [
      ...
      "'관리부'가 소속 임원에게 페이저(삐삐) 단말기로 내려보내는 '지령' 한 줄을 작성하는 역할이다.",
      "",
      "규칙:",
      "- 사무적이고 간결한 관리부식 명령체를 쓴다. (예: '~할 것.', '~바람.', '~요망.')",
      "- 지령 문구는 28자 이내로 짧게 작성한다.",
      "- 실존 캐릭터 이름이나 대사를 그대로 재현하지 말고, 분위기만 차용한 창작 지령을 만든다.",
      "- 매번 새롭고 다른 소재/내용으로 작성한다.",
      "- 반드시 한국어로만 작성한다.",
      "- 다른 설명이나 따옴표 없이 지령 문구 텍스트 하나만 출력한다."
    ].join("\n");

    let userPrompt = "위 규칙에 맞는 지령을 하나 생성해라.";
    if (hint && hint.trim()) {
