...
class PagerApp {
  ...
  advanceRyuenStory() {
    ...
    this.startStoryDecoding(line.text || "");
  ...
  // 기본 모드의 DECODING 연출(암호문 → 실제 글자로 서서히 복호화)을 그대로 재사용해서
  // [뤼엔의 이야기] 대사도 같은 글리치 느낌으로 보여준다.
  startStoryDecoding(targetText) {
    this.clearTimers();

    if (!targetText) {
      this.dom.displayMain.textContent = "";
      return;
    }
